...
CREATE VIEW COURSE_VW AS SELECT COU_ID, e.YEAR_ID, YEAR_LEVEL, r.ROOM_ID, ROOM_NAME, t.TYPE_ID, TYPE_NAME, p.PER_ID, PER_CODE FROM course c, period p, room r, educationyear e, educationtype t WHERE c.YEAR_ID = e.YEAR_ID AND t.TYPE_ID = e.TYPE_ID AND c.PER_ID = p.PER_ID AND c.ROOM_ID = r.ROOM_ID;
